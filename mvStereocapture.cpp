...
static std::atomic<bool> threadRunFlags[3];
...
//-----------------------------------------------------------------------------
//
unsigned int imageAcquisitionStereoThread(void* pData, unsigned int camID,
	...
	atomic<bool> &threadRun, void* pThreadParams)
{
	...
	mvStereoCaptureParameters* pTreadParams = reinterpret_cast<mvStereoCaptureParameters*>(pThreadParams);
...
bool startStereoImageAcquisitions(mvStereoCaptureParameters &mvCapHandler, DeviceData* pMasterdev, DeviceData* pSlavedev)
{
	...
	try
	{
		if (mvCapHandler.mvCapParams.extSyncEnabled == false)
		{
			...
			setupFrameTimer(pMasterdev, mvCapHandler.mvCapParams.triggerFrequency_Hz);
		}

		//make sure frame trigger signals are disabled by default.
		disableFrameTriggerOutput(pMasterdev, 0);
		disableFrameTriggerOutput(pMasterdev, 1);
	}
	...
	threads[0] = std::thread(imageAcquisitionStereoThread, pMasterdev, 0,
	...
	threads[1] = std::thread(imageAcquisitionStereoThread, pSlavedev, 1,
		"slave", mvCapHandler.mvCapParams.nImagePairs,
		mvCapHandler.mvCapParams.decimFactor,
		mvCapHandler.mvCapParams.writeLogEnabled,
		mvCapHandler.mvCapParams.logInterval,
		mvCapHandler.mvCapParams.displayStatsEnabled,
		mvCapHandler.mvCapParams.displayInterval,
		std::ref(threadRunFlags[1]), &mvCapHandler);

	{
		...
		setupFrameTriggerInput(pMasterdev, mvCapHandler.mvCapParams.slaveTrigIn);
		setupFrameTriggerInput(pSlavedev, mvCapHandler.mvCapParams.slaveTrigIn);

		if (mvCapHandler.mvCapParams.extSyncEnabled == true) {
			setupSyncTriggerInput(pMasterdev, mvCapHandler.mvCapParams.extTrigIn);
		}
		//enable trigger pulses when both cameras are ready.
		std::this_thread::sleep_for(std::chrono::milliseconds(100));
		if (mvCapHandler.mvCapParams.extSyncEnabled == false) {
			//enable master trigger, timer2 controlled
			enableFrameTriggerOutput(pMasterdev, mvCapHandler.mvCapParams.masterTrigOut, "Timer2Active");
		}
		else {
			//enable master trigger, controlled by PPS (through mvLogicGates)
			//enableFrameTriggerOutput( devices[0],masterTrigOut);
			enableFrameTriggerOutput(pMasterdev, mvCapHandler.mvCapParams.extTrigOut, "mvLogicGateOR1Output");
		}
	}
#if ENABLE_LOG_DUMP
...
bool endStereoImageAcquisition(mvStereoCaptureParameters &mvCapHandler, DeviceData* pMasterdev, DeviceData* pSlavedev)
{
	...
	if (mvCapHandler.mvCapParams.extSyncEnabled == false) {
		disableFrameTriggerOutput(pMasterdev, mvCapHandler.mvCapParams.masterTrigOut);
	}
	else {
		disableFrameTriggerOutput(pMasterdev, mvCapHandler.mvCapParams.extTrigOut);
	}
